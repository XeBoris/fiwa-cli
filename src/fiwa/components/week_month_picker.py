...
class WeekMonthWidget(Widget):
    ...
    class PeriodChanged(Message):
        ...
        def __init__(self, period_type: str, year: int, week: int = None, month: int = None) -> None:
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(classes="week-month-picker-container"):
            ...
            with Grid(id="week-month-picker-grid", classes="week-month-picker"):
                yield Button("◀", id="wm-period-prev",
                             variant="default",
                             classes="wm-nav-button",
                             compact=True)
                yield Button("▶", id="wm-period-next",
                             variant="default",
                             classes="wm-nav-button",
                             compact=True)
                yield Static(str(self.current_year),
                             id="wm-period-year",
                             classes="wm-display")

                # Display week or month based on period_type
                if self.period_type == "week":
                    yield Static(f"W{self.current_week}", id="wm-period-value", classes="wm-display")
                else:
                    yield Static(f"M{self.current_month:02d}", id="wm-period-value", classes="wm-display")
